...
public class GameMenu extends GameMode {
    ...
    public void run() {
        ...
        desiredDirection.toUnitVector().scale(fighter.getMaxAcceleration());


        fighter.setAcceleration(desiredDirection);

        fighter.movementVelocity.update(fighter.getAcceleration());
        fighter.limitVelocity();

        fighter.addFriction();
        fighter.updatePosition(fighter.getTotalVelocity());

        if(nearestEnemy != null)
            fighter.setDirection(fighter.getDirection().update(fighter.getPosition().differenceVector(nearestEnemy.getPosition()).toUnitVector().scale(.05)));
